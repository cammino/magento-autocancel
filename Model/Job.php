...
class Cammino_Autocancel_Model_Job
{
    public function autocancel()
    {
        ...
        $selectedMethods = Mage::getStoreConfig('autocancel/general/payment_methods');
        foreach ($selectedMethods as $method) {
            Mage::log('Checando pedidos status pending e com o método: ' . $method, null, 'autocancel.log');
            $paymentCollection =  Mage::getModel('sales/order_payment')->getCollection()->addFieldToFilter('method', $method)->addFieldToFilter('status', array('in' => array('pending')));
